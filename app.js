...
let catalog = [
  ...
  { id: 'v3', name: 'Fresh Spinach (Palak)', price: 20, unit: '1 bunch', emoji: '🥬', category: 'Leafy' },
  { id: 'v4', name: 'Red Onions', price: 45, unit: '1 kg', emoji: '🧅', category: 'Vegetables' },
  { id: 'v5', name: 'Green Chillies', price: 15, unit: '250 g', emoji: '🌶️', category: 'Vegetables' }
];
...
window.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    let user = null;
    try {
      user = JSON.parse(localStorage.getItem('vegmart_user'));
    } catch (e) {
...
async function loadAll() {
  ...
  if (user.address && document.getElementById('headerAddrDisplay')) {
    document.getElementById('headerAddrDisplay').textContent = `Deliver to: ${user.address}`;
  }
  if (user.name && document.getElementById('profileAvatarChar')) {
    document.getElementById('profileAvatarChar').textContent = user.name.charAt(0).toUpperCase();
  }

  if (supabaseClient) {
    try {
      const { data: veg, error } = await supabaseClient.from('vegetables').select('*').order('name');
      if (!error && veg && veg.length > 0) {
        catalog = veg;
      }
    } catch (e) {
      console.warn("Using fallback local catalog.");
    }
  }
...
function renderCategories() {
  const container = document.getElementById('appContent');
  if (!container) return;
  const cats = [...new Set(catalog.map(v => v.category || 'Produce'))];
